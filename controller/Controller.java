package Qn_9to12.controller;

import Qn_9to12.Database.DatabaseConnection;
...
public class Controller {
    ...
    public int addTask(TaskModel taskModel){
        try{
            String query ="insert into task(task,taskId) values(?,?)";
    ...
    public int addJob(JobModel jobModel){
        try{
            String query="insert into tblJob(jobId, jobName,jobProfit, deadline, numOfTask) values(?,?,?,?,?)";
    ...
    public int addJobTask(JobTaskModel jobTaskModel){
        try{
            String query="insert into tblJobTask(jobId, sourcetask, destinationtask) values(?,?,?)";
            PreparedStatement st= db.connection.prepareStatement(query);
    ...
    public ArrayList<JobTaskModel> fetchTaskModels(int jobId){
        ...
        try{
            String query = "select * from tbljobtask where jobId=?";
            PreparedStatement ps = db.connection.prepareStatement(query);
            ps.setInt(1,jobId);
            ResultSet resultSet = db.retrieve(ps);
